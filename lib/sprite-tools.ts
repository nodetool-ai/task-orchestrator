...
export const SPRITE_API_DESCRIPTORS = [
  ...
  operation("snapshots", "checkpoint", "Asynchronously save a personal filesystem checkpoint of a Sprite run you own. Supply a stable operationId and inspect checkpointStatus. Retries with the same inputs are idempotent. Pause writers first for consistency. This may contain run credentials and is never promoted to the shared pool.",
    Type.Object({ ...target, operationId: Type.String({ format: "uuid", description: "Stable UUID for this checkpoint request; reuse it on retries." }), comment: Type.Optional(Type.String({ maxLength: 1000 })) }), "create",
    async (params, ctx) => (await import("./sprite-commands")).checkpointRun(ctx, params)),
  operation("snapshots", "checkpointStatus", "Inspect an asynchronous personal checkpoint by its stable operationId. Failed and unknown operations can be retried safely with the same checkpoint request.",
    Type.Object({ ...target, operationId: Type.String({ format: "uuid" }) }), "read",
    async (params, ctx) => (await import("./sprite-commands")).checkpointRunStatus(ctx, params)),
