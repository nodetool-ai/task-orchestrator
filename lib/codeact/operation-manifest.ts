...
const TOOL_OPS: OperationEntry[] = [
  ...
  t("sessions", "await_session", "Park until a session is terminal", "app.sessions.await"),
  ...
  t("events", "events__poll", "Non-blocking inbox poll", "app.events.poll"),
...
const REST_OPS: OperationEntry[] = [
  ...
  rx("system", "GET /api/health", "Liveness/readiness probe",
    "Ops endpoint; no product effect."),
  rx("system", "GET /api/metrics", "Prometheus metrics scrape",
    "Ops endpoint; no product effect."),
  ...
  rx("system", "POST /api/environments/build", "Kick a Docker environment build",
    "Infra/CI image build; out of scope for the guest SDK in v1 (may be added later as an admin capability)."),
  rx("runs", "GET /api/runs/:id/events", "SSE live run event stream",
    "Server-Sent-Events streaming; the guest consumes run state via events__poll, not a raw SSE socket."),
  rx("runs", "POST /api/runs/:id/messages", "SSE post-and-stream a run reply",
    "SSE streaming chat turn; guest-to-run messaging is spawn__append_message."),
  ...
  rx("sessions", "GET /api/sessions/:id/events", "SSE session event stream",
    "SSE streaming; guest reads the event tail via get_session."),
  rx("chats", "POST /api/chats/:id/messages", "SSE post-and-stream a chat reply",
    "SSE streaming chat turn; interactive UI/human surface."),
  rx("system", "GET /api/inbox", "Global needs-you inbox projection",
    "Cross-run UI projection for humans; guest uses events__poll within its own tree."),
