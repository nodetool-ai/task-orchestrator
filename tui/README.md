...
| `tab` | complete the command word, then address the next waiting agent; completes a `/new` persona or `/model` suggestion while one is up | — | — | — |
| `↑` `↓` | recall history (walk argument suggestions while one is up) | move | move | move |
