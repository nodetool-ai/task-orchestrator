...
# apps land where the database does. No-op when FLY_REGION is the default.
sed -i.bak "s/^primary_region = .*/primary_region = \"$REGION\"/" fly.toml fly.runner.toml && rm -f fly.toml.bak fly.runner.toml.bak
...
# ── 2. Database ──────────────────────────────────────────────────────────────
bold "--- 2/6  Provisioning database ---"
...
bold "--- 3/6  Building + pushing runner image ---"
...
[[ -n "${GH_TOKEN:-}" ]]                 && secret_args+=( GH_TOKEN="$GH_TOKEN" )
